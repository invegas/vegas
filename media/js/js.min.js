...
//myself
prettyPrint();var category=$("body").attr("data-category");$('#category option[value="'+category+'"]').attr("selected",true);$(".edit-end").click(function(){window.location="/blog/"+$(".title").attr("data-origin")});$(".edit-save").click(function(){var c=$(this);var b=$(".title");var a=$("#category").val();b.attr("data-origin",b.text());$(this).text("保存中");$.ajax({url:"/editBlog",type:"post",data:{title:b.text(),content:$(".content").html(),id:$("body").attr("data-id"),category:a},success:function(d){if(d.status=="ok"){$(".save-status").show();$(".save-status-success").show();$(".save-status-failed").hide();c.text("保存");$(".info-tag").text(a)}else{$(".save-status").show();$(".save-status-success").hide();$(".save-status-failed").show();c.text("保存")}}})});